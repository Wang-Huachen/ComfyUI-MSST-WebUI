...
// ── 同步加载模型数据 ──
const MODEL_DATA_URL = new URL("model_data.json", import.meta.url).href;
var modelData = null;
try {
...
// ── 输出同步（[0]=model_info, [1]=AUDIO_0, [2]=STRING_0, ...） ──
...
app.registerExtension({
    ...
    nodeCreated: function(node) {
        ...
        // 延迟同步（widget 值需要 configure 恢复后才能读取）
        requestAnimationFrame(function() {
            if (cls === "MSSTSeparate") _filterModels(node);
            _syncOutputs(node);
        });
    }
});
